...
TEST_FN(st_push) {
  ...
  TEST_ASSERT(ciao_st_push(&st, 'a'));
  TEST_ASSERT(st.len == 1);
  TEST_ASSERT(st.items[0] == 'a');
  
  TEST_ASSERT(ciao_st_push(&st, 'b'));
...
TEST_FN(st_push_zstr) {
  ...
  TEST_ASSERT(ciao_st_push_zstr(&st, "hello"));
  TEST_ASSERT(st.len == 5);
  TEST_ASSERT(st.items[0] == 'h');
  TEST_ASSERT(st.items[4] == 'o');
  
  TEST_ASSERT(ciao_st_push_zstr(&st, " world"));
  TEST_ASSERT(st.len == 11);
  
  TEST_ASSERT(ciao_st_push_zstr(&st, NULL));
  TEST_ASSERT(st.len == 11);
...
TEST_FN(st_push_buf) {
  ...
  TEST_ASSERT(ciao_st_push_buf(&st, buf, 4));
...
TEST_FN(st_push_fmt) {
  Ciao_Strut st = {0};
  TEST_ASSERT(ciao_st_reserve_exact(&st, CIAO_ST_INIT_CAP));

  st.len = 0;
  ciao_st_push_fmt(&st, "%d", 42);
  ...
  st.len = 0;
  ciao_st_push_fmt(&st, "%u", 18446744073709551615ULL);
  ...
  st.len = 0;
  ciao_st_push_fmt(&st, "%c", 'E');
  TEST_CHECK(st.len == 1 && ciao_st_get(&st, 0) == 'E') {
    log_info("St contents: |%.*s|", (int)st.len, st.items);
    log_info("St len: %zu", st.len);
  }

  {
    ...
    ciao_st_push_fmt(&st, "%.*s", (int)sv.count, sv.data);
...
TEST_FN(st_push_si64) {
  ...
  TEST_ASSERT(ciao_st_push_si64(&st, 0));
  TEST_ASSERT(st.len == 1);
  TEST_ASSERT(st.items[0] == '0');

  st.len = 0;
  TEST_ASSERT(ciao_st_push_si64(&st, -12345));
...
TEST_FN(st_push_si64_min) {
  ...
  TEST_ASSERT(ciao_st_push_si64(&st, val));
  TEST_ASSERT(st.items[0] == '-');
...
TEST_FN(st_push_ui64) {
  ...
  TEST_ASSERT(ciao_st_push_ui64(&st, 0));
  TEST_ASSERT(st.items[0] == '0');
  
  st.len = 0;
  TEST_ASSERT(ciao_st_push_ui64(&st, 18446744073709551615ULL));
...
TEST_FN(st_push_hex) {
  ...
  TEST_ASSERT(ciao_st_push_hex(&st, 0, false));
  TEST_ASSERT(st.items[0] == '0');
  
  st.len = 0;
  TEST_ASSERT(ciao_st_push_hex(&st, 255, false));
  TEST_ASSERT(st.items[0] == 'f');

  st.len = 0;
  TEST_ASSERT(ciao_st_push_hex(&st, 255, true));
  TEST_ASSERT(st.items[0] == 'F');
...
TEST_FN(st_push_bool) {
  ...
  TEST_ASSERT(ciao_st_push_bool(&st, true));
  TEST_ASSERT(st.len == 4);
  TEST_ASSERT(strncmp(st.items, "true", 4) == 0);
  
  
  st.len = 0;
  TEST_ASSERT(ciao_st_push_bool(&st, false));
  TEST_ASSERT(st.len == 5);
...
TEST_FN(st_reserve) {
  ...
  TEST_ASSERT(ciao_st_reserve(&st, 100));
...
TEST_FN(st_ensure) {
  ...
  TEST_ASSERT(ciao_st_ensure(&st, 50));
...
TEST_FN(st_reserve_exact) {
  Ciao_Strut st = {0};
  ciao_st_push_zstr(&st, "hello");
  
  TEST_ASSERT(ciao_st_reserve_exact(&st, 10));
